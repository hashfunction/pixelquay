...
public sealed class TextTool : BaseTool
{
	...
	{
		// Keep model-owned content options in sync during ordinary drawing. Export can
		// then clone them without touching the live tool, history or GTK widgets.
		CurrentTextEngine.RenderOptions = new TextRenderOptions (
			UseAntialiasing, FillText, StrokeText, BackgroundFill, OutlineWidth,
			PangoCairo.Functions.ContextGetResolution (CurrentTextLayout.Layout.GetContext ()),
			selection?.Clone (),
			useTextLayer && (showCursor || is_editing || ctrl_key || CurrentTextEngine.HasSelection ()));
		...
		TextContentRenderer.Draw (g, CurrentTextLayout, CurrentTextEngine.RenderOptions);
		selection?.Clip (g);
