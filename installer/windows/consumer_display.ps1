...
function Add-PixelQuayConsumerDisplayTypes {
    if('PixelQuayConsumerDisplay.DisplayModes' -as [type]){return}
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.InteropServices;
namespace PixelQuayConsumerDisplay {
...
function Convert-PixelQuayConsumerMode($Mode){
...
function Get-PixelQuayConsumerModeChoice($Modes){
...
function Get-PixelQuayConsumerDisplayState([string]$Device){
    Add-PixelQuayConsumerDisplayTypes
    if(-not $Device){$Device=[PixelQuayConsumerDisplay.DisplayModes]::Primary()}
    return @{device=$Device;current=[PixelQuayConsumerDisplay.DisplayModes]::Current($Device);modes=[PixelQuayConsumerDisplay.DisplayModes]::Supported($Device)}
}
function Set-PixelQuayConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [PixelQuayConsumerDisplay.DisplayModes]::Change($Device,$Mode,$Flags)
}

function Start-PixelQuayConsumerDisplay($State){
    $snapshot=Get-PixelQuayConsumerDisplayState
    $State.displayOriginalMode=$snapshot.current;$State.displayDevice=$snapshot.device
    $State.displayEvidence=[ordered]@{purpose='actual native display only';device=$snapshot.device;before=(Convert-PixelQuayConsumerMode $snapshot.current);
        supported_modes=@($snapshot.modes|ForEach-Object {Convert-PixelQuayConsumerMode $_});selected=$null;test_result=$null;apply_result=$null;after=$null;
    ...
    if($snapshot.current.dmPelsWidth -ge 1472 -and $snapshot.current.dmPelsHeight -ge 1080 -and $snapshot.current.dmBitsPerPel -eq 32){
        $State.displayEvidence.after=Convert-PixelQuayConsumerMode $snapshot.current;return
    }
    $selected=Get-PixelQuayConsumerModeChoice $snapshot.modes;$State.displayEvidence.selected=Convert-PixelQuayConsumerMode $selected
    $test=Set-PixelQuayConsumerDisplayMode $snapshot.device $selected 2;$State.displayEvidence.test_result=$test
    ...
    $apply=Set-PixelQuayConsumerDisplayMode $snapshot.device $selected 0;$State.displayEvidence.apply_result=$apply
    if($apply -ne 0){throw "Native dynamic display change failed ($apply)."}
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $after=(Get-PixelQuayConsumerDisplayState $snapshot.device).current
    ...
    $State.displayEvidence.after=Convert-PixelQuayConsumerMode $after
...
function Restore-PixelQuayConsumerDisplay($State){
    if(-not $State.displayOriginalMode){return}
    if($State.displayRestoreRequired){
        $result=Set-PixelQuayConsumerDisplayMode $State.displayDevice $State.displayOriginalMode 0;$State.displayEvidence.restore_result=$result
        if($result -ne 0){throw "Native display restoration failed ($result)."}
    }
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $observed=Convert-PixelQuayConsumerMode (Get-PixelQuayConsumerDisplayState $State.displayDevice).current
