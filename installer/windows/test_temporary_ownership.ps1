...
$script:probeRoot=Join-Path ([IO.Path]::GetTempPath()) ('pixelquay-temporary-test-'+[guid]::NewGuid().ToString('N'))
...
try {
    $failure=$null
    try { Invoke-PixelQuayInstallQualification unused unused unused $script:probeRoot | Out-Null }
