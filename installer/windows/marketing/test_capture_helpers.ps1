...
function Snapshot {return @{process_id=41;main_pid=41;target_pid=41;main=100;target=200;foreground=200;main_title='Cedar Coast.png - TintFable';target_title='Export with Recipe';main_visible=$true;target_visible=$true;dpi=96;
    main_maximized=$true;main_bounds=@(-8,-8,1936,1048);capture_bounds=@(0,0,1920,1032);target_bounds=@(400,180,1050,650);work_area=@(0,0,1920,1032);desktop=@(0,0,1920,1080)}}
...
foreach($mutation in @('pid','foreign-main','foreign-target','foreground','title','hidden-main','hidden-target','dpi','width','desktop','clipped-modal','not-maximized','capture-crop')){
    $v=Snapshot
    switch($mutation){pid{$v.process_id=8};foreign-main{$v.main_pid=8};foreign-target{$v.target_pid=8};foreground{$v.foreground=100};title{$v.target_title='Foreign'};hidden-main{$v.main_visible=$false};hidden-target{$v.target_visible=$false};dpi{$v.dpi=144};width{$v.main_bounds[2]=1400};desktop{$v.desktop[2]=1600};clipped-modal{$v.target_bounds[0]=-1};not-maximized{$v.main_maximized=$false};capture-crop{$v.capture_bounds[0]=1}}
...
Write-Output 'PASS real full-window capture, thirteen ownership/geometry refusals, mid-frame change, and eight lifecycle failure boundaries.'
