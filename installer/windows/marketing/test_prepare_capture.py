# Copyright 2026 Trieflow LLC. MIT. Exact artifact download/extraction boundary.
import io
...
BOUND={'workflow_run_id':'12','source_commit':'b'*40}

class ArtifactTests(unittest.TestCase):
    def test_historical_binding_blocks_before_any_download_or_output_creation(self):
        with tempfile.TemporaryDirectory() as directory,patch.object(checks,'read_json',return_value={'schema_version':1,'product':'TintFable','qualified':None}),patch.object(prepare,'gh_json',side_effect=AssertionError('network called')),patch.object(prepare.subprocess,'check_output',side_effect=AssertionError('checkout read')):
    ...
    def run_archive(self,entries,*,members=None,limit=100000,change=None):
        ...
        info={'id':42,'name':'fixture','expired':False,'size_in_bytes':len(raw),
            'workflow_run':{'id':int(BOUND['workflow_run_id']),'head_sha':BOUND['source_commit']}}
        ...
        with tempfile.TemporaryDirectory() as directory,patch.object(prepare,'gh_json',return_value=info),patch.object(prepare.subprocess,'run',side_effect=download):
            target=Path(directory)/'artifact';prepare.artifact(42,'fixture',target,limit,BOUND,members)
    ...
    def test_production_download_extraction_requires_exact_store_members_and_current_artifact(self):
        ...
        for change in ({'expired':True},{'id':43},{'workflow_run':{'id':int(BOUND['workflow_run_id']),'head_sha':'a'*40}}):
