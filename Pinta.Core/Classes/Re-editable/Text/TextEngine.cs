...
public sealed partial class TextEngine
{
	...
	// Captured during normal tool redraw, never read from GTK controls during export.
	public TextRenderOptions RenderOptions { get; set; } = new ();
	...
	public TextEngine Clone ()
	{
		TextEngine clonedTE = new () {
			lines = [.. lines],
			State = State,
			RenderOptions = RenderOptions.Copy (),
